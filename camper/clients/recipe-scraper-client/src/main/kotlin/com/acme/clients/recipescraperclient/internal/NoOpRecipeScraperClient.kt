...
internal class NoOpRecipeScraperClient : RecipeScraperClient {
    ...
    override fun scrapeImages(param: ScrapeRecipeImagesParam): Result<ScrapedRecipe, AppError> {
        logger.info(
            "NoOpRecipeScraperClient: would read {} photo(s) ({})",
            param.images.size, param.images.joinToString { "${it.mediaType} ${it.base64Data.length} chars" }
        )
        return success(cannedRecipe())
    }

    private fun cannedRecipe() = ScrapedRecipe(
