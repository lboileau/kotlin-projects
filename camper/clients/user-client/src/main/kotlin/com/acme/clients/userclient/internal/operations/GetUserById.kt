...
internal class GetUserById(private val jdbi: Jdbi) {
    ...
    fun execute(param: GetByIdParam): Result<User, AppError> {
        ...
        val entity = jdbi.withHandle<User?, Exception> { handle ->
            val user = handle.createQuery("SELECT id, external_id, email, username, experience_level, avatar_seed, profile_completed, created_at, updated_at FROM users WHERE id = :id")
                .bind("id", param.id)
