...
/**
 * Updates a manual item's purchased quantity, scoped to [UpdateManualItemPurchaseParam.mealPlanId]
 * — an item id that exists but belongs to a different meal plan is treated the same as a
 * nonexistent one (`NotFoundError`).
 */
internal class UpdateManualItemPurchase(private val jdbi: Jdbi) {
    ...
    fun execute(param: UpdateManualItemPurchaseParam): Result<ShoppingListManualItem, AppError> {
        ...
        logger.debug("Updating manual item purchase id={} mealPlanId={} quantityPurchased={}", param.id, param.mealPlanId, param.quantityPurchased)
        val entity = jdbi.withHandle<ShoppingListManualItem?, Exception> { handle ->
            handle.createQuery(
                ...
                WHERE id = :id AND meal_plan_id = :mealPlanId
