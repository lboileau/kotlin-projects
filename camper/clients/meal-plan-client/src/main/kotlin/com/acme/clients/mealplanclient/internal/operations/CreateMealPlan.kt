        ...
        return try {
            val entity = jdbi.withHandle<MealPlan, Exception> { handle ->
                ...
                handle.createUpdate(
                    ...
                    VALUES (:id, CAST(:planId AS uuid), :name, :servings, :scalingMode, :isTemplate, CAST(:sourceTemplateId AS uuid), :createdBy, :createdAt, :updatedAt)
                    """.trimIndent()
                )
                    .bind("id", id)
                    ...
                    .bind("sourceTemplateId", param.sourceTemplateId?.toString())
