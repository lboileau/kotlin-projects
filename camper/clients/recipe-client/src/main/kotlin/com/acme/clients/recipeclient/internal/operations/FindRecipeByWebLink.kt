package com.acme.clients.recipeclient.internal.operations

import com.acme.clients.common.Result
import com.acme.clients.common.error.AppError
import com.acme.clients.common.success
import com.acme.clients.recipeclient.api.FindByWebLinkParam
import com.acme.clients.recipeclient.internal.adapters.RecipeRowAdapter
import com.acme.clients.recipeclient.internal.validations.ValidateFindRecipeByWebLink
import com.acme.clients.recipeclient.model.Recipe
import org.jdbi.v3.core.Jdbi
import org.slf4j.LoggerFactory

internal class FindRecipeByWebLink(private val jdbi: Jdbi) {
    private val logger = LoggerFactory.getLogger(FindRecipeByWebLink::class.java)
    private val validate = ValidateFindRecipeByWebLink()

    fun execute(param: FindByWebLinkParam): Result<Recipe?, AppError> {
        val validation = validate.execute(param)
        if (validation is Result.Failure) return validation

        logger.debug("Finding recipe by webLink={}", param.webLink)
        val entity = jdbi.withHandle<Recipe?, Exception> { handle ->
            handle.createQuery(
                "SELECT id, name, description, web_link, base_servings, status, created_by, duplicate_of_id, created_at, updated_at FROM recipes WHERE web_link = :webLink"
            )
                .bind("webLink", param.webLink)
                .map { rs, _ -> RecipeRowAdapter.fromResultSet(rs) }
                .findOne()
                .orElse(null)
        }
        return success(entity)
    }
}
