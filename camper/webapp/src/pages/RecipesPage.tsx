  ...
  return (
    <div className="recipes-list-view">
      ...
      <div className="recipes-toolbar">
        ...
        <button className="recipes-create-btn" onClick={() => navigate('/recipes/new')}>
...
function RecipeDetailView({
  ...
  useEffect(() => {
    ...
    api.getRecipe(recipeId)
      .then(detail => setSelectedRecipe(detail))
  ...
  const handlePublish = async () => {
    ...
    setResolveError('');
    try {
      await api.publishRecipe(selectedRecipe.id);
      onRecipesMutated();
      await refreshDetail();
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      setDetailError(err instanceof Error ? err.message : 'Failed to delete recipe');
    } finally {
  ...
  if (notFound) {
    return (
      <div className="recipes-detail-view">
        ...
        <div className="recipes-empty" style={{ marginTop: 'var(--space-xl)' }}>
          <svg width="72" height="72" viewBox="0 0 72 72" className="recipes-empty-icon">
            <rect x="12" y="16" width="48" height="40" rx="4" fill="none" stroke="var(--tan-deep)" strokeWidth="2" />
            <line x1="22" y1="28" x2="50" y2="28" stroke="var(--tan-deep)" strokeWidth="1.5" strokeLinecap="round" opacity="0.6" />
            <line x1="22" y1="35" x2="50" y2="35" stroke="var(--tan-deep)" strokeWidth="1.5" strokeLinecap="round" opacity="0.4" />
          ...
          <p className="recipes-empty-text">Recipe not found.</p>
          <Link to="/recipes" className="recipes-back-btn" style={{ display: 'inline-flex', marginTop: 'var(--space-md)', textDecoration: 'none' }}>
            ← Back to recipes
          </Link>
        </div>
      </div>
  ...
  const pendingCount = selectedRecipe.ingredients.filter(i => i.status === 'pending_review').length;
      ...
      <div className="recipes-detail-view">
        <div className="recipes-detail-header">
          ...
          <div className="recipes-detail-actions">
            <button className="recipes-action-btn recipes-action-btn--edit" onClick={() => navigate(`/recipes/${selectedRecipe.id}/edit`)}>
              <svg width="15" height="15" viewBox="0 0 15 15">
        ...
        {detailError && <p className="recipes-error">{detailError}</p>}
...
// ── RecipeCreateView ─────────────────────────────────────────────────────────
// Thin wrapper: form state and logic now live in RecipeForm. This wrapper
// provides the page-level shell (back button) and the navigate-on-success callback.
...
function RecipeImportView({ onRecipesMutated }: { onRecipesMutated: () => void }) {
  const navigate = useNavigate();
      ...
      <div className="recipes-form-container">
        ...
        <form className="recipes-form" onSubmit={handleImport}>
          ...
          <div className="recipes-form-actions">
            <button type="button" className="recipes-cancel-btn" onClick={() => navigate('/recipes')}>
              Cancel
            </button>
            <button
              type="submit"
...
function RecipeEditView({ onRecipesMutated }: { onRecipesMutated: () => void }) {
  const { recipeId } = useParams<{ recipeId: string }>();
  const navigate = useNavigate();
  ...
  useEffect(() => {
    ...
    api.getRecipe(recipeId)
      .then(detail => {
        ...
        setEditServings(detail.baseServings);
        setEditMeal(detail.meal || '');
        setEditTheme(detail.theme || '');
      })
  ...
  if (editLoading) {
    return (
      <div className="recipes-create-view">
        <div className="recipes-detail-header">
          <button className="recipes-back-btn" onClick={() => navigate(recipeId ? `/recipes/${recipeId}` : '/recipes')}>
            <svg width="18" height="18" viewBox="0 0 18 18">
              <path d="M11,4 L6,9 L11,14" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
            Cancel
    ...
    <div className="recipes-create-view">
      ...
      <div className="recipes-form-container">
        ...
        <form className="recipes-form" onSubmit={handleSaveEdit}>
          ...
          <div className="recipes-form-actions">
            <button type="button" className="recipes-cancel-btn" onClick={() => navigate(recipeId ? `/recipes/${recipeId}` : '/recipes')}>
              Cancel
            </button>
            <button
              type="submit"
...
export function RecipesPage() {
  ...
  const onRecipesMutated = useCallback(async () => {
    try {
      const r = await api.getRecipes();
      setRecipes(r);
    } catch {
      // silent — list may be briefly stale
    }
  }, []);
      ...
      <div className="recipes-content">
        ...
        <Routes>
          ...
          <Route
            ...
            element={
              <RecipeCreateView
                ingredients={ingredients}
                setIngredients={setIngredients}
                onRecipesMutated={onRecipesMutated}
              />
            }
          />
          <Route
            path="import"
            element={
              <RecipeImportView onRecipesMutated={onRecipesMutated} />
            }
          />
          <Route
            path=":recipeId"
          ...
          <Route
            path=":recipeId/edit"
            element={
              <RecipeEditView onRecipesMutated={onRecipesMutated} />
            }
          />
