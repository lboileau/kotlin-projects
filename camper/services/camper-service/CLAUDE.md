...
- **DTOs:**
  - Requests: `CreateRecipeRequest`, `ImportRecipeRequest(url)`, `ImportRecipeFromImagesRequest(images: [ImportImageRequest(mediaType, data)])` (1–3 photos, `data` is raw base64 — no `data:` prefix — sent inside the JSON body; there is no multipart handling anywhere in the app), `UpdateRecipeRequest(name?, description?, baseServings?, meal?, theme?)`, `ResolveIngredientRequest(action, ingredientId?, newIngredient?)`, `ResolveDuplicateRequest(action)`
...
- **Actions:**
  ...
  - `ImportRecipeFromImagesAction`: validates 1–3 photos, media type in `RecipeImage.SUPPORTED_MEDIA_TYPES`, raw base64 (rejects a `data:` URL), ≤ 5 MB decoded (the Claude API's per-image limit — checked on the base64 length *before* decoding); calls `RecipeScraperClient.scrapeImages`; no webLink dedupe and the draft gets `webLink = null` (the unique index is partial on `web_link IS NOT NULL`, so photo imports never 409)
  - `ScrapedRecipeDrafter`: the part shared by both imports — loads the catalogue as `ExistingIngredient`s, detects a similar-named recipe, creates the `draft`, adds every line as `approved` (clean HIGH match) or `pending_review` (any flag), reads the detail back with favourite summaries
...
- **Routes:** (all require `X-User-Id` header)
  ...
  - `POST /api/recipes/import-images` — import recipe from 1–3 photos (201; 400 on count/media type/base64/size, 422 `SCRAPE_FAILED` when no recipe could be read)
