...
2. **Favourite button — recipe page only.** No tappable heart on list rows; rows keep their single ＋ action. Each row shows a **read-only** `♥ N` (small filled heart + count, not a button, not focusable, accessible text like "3 favourites") in its secondary line beside "Serves 2", only when `favoriteCount > 0`. The owner called this count "critical" — it is a requirement, not an optional extra.
3. **Recipe page — heart across from the title, and WHO inline.**
   - The heart toggle sits **right-justified across from the recipe title**, aligned to the title's first line. `components/PageHero.tsx` gets an optional `action?: ReactNode` rendered in a flex row with the `<h1>` (`flex: 1; min-width: 0`, the title still WRAPS as it does today — never a one-line ellipsis); other `PageHero` call sites must render identically without it. 44px target, icon-only (`HeartIcon` / `HeartFilledIcon`, accent colour), `aria-pressed`, aria-label "Favourite" / "Remove from favourites". The title must not shift sideways when the heart appears after loading.
