...
PostgreSQL database for the Interview Manager project.
...
```bash
cd databases/interview-manager-db
docker-compose up -d
```
...
```bash
psql -h localhost -p 5433 -U postgres -d interview_manager_db
```
...
```bash
psql -h localhost -p 5433 -U postgres -d interview_manager_db -f seed/dev_seed.sql
```
